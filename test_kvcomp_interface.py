...
repre_index = torch.arange(0, s, dtype = torch.int32).cuda()
q_index = torch.arange(0, s, dtype = torch.int32).cuda()
q_index = q_index % b
...
batch_offset = torch.arange(0, s, math.ceil(s / b), dtype=torch.int32).cuda()
batch_offset = torch.cat([batch_offset, torch.tensor([s], dtype=torch.int32).cuda()])
